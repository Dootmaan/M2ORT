...
class M2ORT(nn.Module):
    ...
    def forward(self, img ,img2,img3):
        # img2=torch.nn.functional.interpolate(img,scale_factor=0.5,mode='bilinear')
        # img3=torch.nn.functional.interpolate(img,scale_factor=0.25,mode='bilinear')
        x1 = self.to_patch_embedding1(img)
        x2 = self.to_patch_embedding2(img2)
        x3 = self.to_patch_embedding3(img3)
